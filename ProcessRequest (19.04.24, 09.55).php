...
echo '</pre>';
Logger::log("Genarated user falcon file name: " .$responseData[0]->file_name);
...
Logger::log("Again check if requested user exists: " .$responseData[0]->file_name);
...
if ($database_gateway->error == '' ) {
    ...
    $mail->FromName = 'boovis by Males GmbH';
    ...
    // NOTE IT WORKS SIMPLE active then will work properly for the images.
    // this is the genared attachment before send file.
    //$mail->addAttachment("generated_image". DIRECTORY_SEPARATOR .$file_name_without_extension);
    ...
    if(!$mail->send()) {
        ...
        if(!empty($responseData[0]->genareted_image)){
            //print_r($responseData);
            $requestedPersonalInfo = unserialize($responseData[0]->user_info);
            ...
            if($biginDataInsertRequest->biginResponseStatus != "DUPLICATE_DATA"){
                ...
                $biginCompanyDataSet = $biginDataInsertRequest->biginInsertUserStructure($requestedPersonalInfo,
                    $responseData[0]->genareted_image);
